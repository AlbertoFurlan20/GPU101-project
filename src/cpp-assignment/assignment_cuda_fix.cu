#include "assignment_cuda_fix.cuh"
...
int main()
{
    ...
    dim3 gridSize((width + blockDim.x - 1) / blockDim.x, (height + blockDim.y - 1) / blockDim.y); // Calculate grid size

    std::cout << "\n[SETUP]:: set block size :=(" << blockDim.x << ", " << blockDim.y << ", " << blockDim.z << ")";
    std::cout << "\n[SETUP]:: set grid size :=(" << gridSize.x << ", " << gridSize.y << ", " << gridSize.z << ")\n";

    convolution_cuda<<<gridSize, blockDim>>>(d_input, d_filter, d_output, width, height, FILTER_SIZE, FILTER_RADIUS);
    ...
    std::cout << "[OUTPUT]\n";
    printDynamicArray(output_gpu, height * width);
